...
onMounted(async () => {
  ...
  console.log('todo', todoStore.list)
...
<template>
  <div>
    ...
    <div class="flex">
      <input class="input input-bordered w-full" type="text" placeholder="text here" v-model="todotext">
      <button class="btn btn-primary ml-2" @click="createTodo(todotext)">add todo</button>
    </div>
    <div>
      <div role="tablist" class="tabs tabs-boxed mt-4">
        <a v-for="status in todoStore.statuses" role="tab" :class="status===selectedStatus ?'tab tab-active':'tab'" @click="changeSelectedStatus(status)">{{status}}</a>
      </div>
    </div>
    <div class="flex items-center justify-between" v-for="todo in filterTodoList" :key="todo.id">
      <div>
